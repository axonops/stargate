...
public class DelegatingAuthorizer extends CassandraAuthorizer {
  ...
  private static final Logger LOG = LoggerFactory.getLogger(DelegatingAuthorizer.class);
  ...
  public Set<Permission> grant(
    ...
    get(stage);
    // Return the permissions that were successfully granted
    return permissions;
  }
  ...
  public Set<Permission> revoke(
    ...
    get(stage);
    // Return the permissions that were successfully revoked
    return permissions;
  }
  ...
  private static AuthorizedResource resource(IResource resource) {
    ...
    if (resource instanceof FunctionResource) {
      ...
      // Log warning and return a generic resource for unsupported types
      LOG.warn("Unsupported resource type: {}. Creating generic resource.", resource.getClass());
      authResource =
          ImmutableAuthorizedResource.of(ResourceKind.KEYSPACE)
              .withKeyspace(EntitySelector.byName(resource.getName()));
    }
