...
public class IdempotencyAnalyzer {
  // In Cassandra 5.0, we need to identify non-idempotent functions differently
  // These are functions that return different values on each call
  private static final Set<String> NON_IDEMPOTENT_FUNCTION_NAMES =
      ImmutableSet.of(
          "now",
          "uuid",
          "currenttimestamp",
  ...
  private static boolean analyzeStatement(CQLStatement statement) {
    ...
    if (statement instanceof ModificationStatement) {
      ...
      for (Function f : modification.getFunctions()) {
        // In Cassandra 5.0, check by function name instead
        String functionName = f.name().name.toLowerCase();
        if (NON_IDEMPOTENT_FUNCTION_NAMES.contains(functionName)) {
          return false;
        }
      }
