...
public class StargateQueryHandler implements QueryHandler {
  ...
  private AtomicReference<AuthorizationService> authorizationService = new AtomicReference<>();
  ...
  private ResultMessage maybeIntercept(
      ...
      Dispatcher.RequestTime queryStartNanoTime) {
    for (QueryInterceptor interceptor : interceptors) {
      ResultMessage result =
          interceptor.interceptQuery(
              // Extract the nanos value from RequestTime - implementation specific
              statement,
              queryState,
              options,
              customPayload,
              queryStartNanoTime == null ? System.nanoTime() : System.nanoTime());
      if (result != null) {
        return result;
      }
    }
  ...
  protected void authorizeByToken(Map<String, ByteBuffer> customPayload, CQLStatement statement) {
    ...
    if (statement instanceof SelectStatement) {
      ...
      // Log warning for unsupported statement types but allow them through
      // This is more lenient for newer statement types in Cassandra 5.0
      logger.warn(
          "Skipping authorization for unsupported statement type: {}",
  ...
  public Optional<AuthorizationService> getAuthorizationService() {
    return authorizationService != null
        ? Optional.ofNullable(authorizationService.get())
        : Optional.empty();
  }
}
