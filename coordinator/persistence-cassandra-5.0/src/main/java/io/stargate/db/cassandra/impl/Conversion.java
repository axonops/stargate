...
public class Conversion {
  ...
  public static ByteBuffer toPagingState(PagingPosition pos, Parameters parameters) {
    ...
    switch (pos.resumeFrom()) {
      ...
      default:
        // Log warning for unsupported paging modes but use partition mode as fallback
        logger.warn(
            "Unsupported paging mode: {}. Using PARTITION mode as fallback.", pos.resumeFrom());
        rowMark = null;
    }
  ...
  public static QueryOptions toInternal(
    ...
    QueryOptions options =
        QueryOptions.create(
            ...
            parameters.nowInSeconds().orElse((int) FBUtilities.nowInSeconds()));
    ...
    for (Map.Entry<InetAddressAndPort, org.apache.cassandra.exceptions.RequestFailureReason> entry :
      ...
      external.put(
          org.apache.cassandra.stargate.locator.InetAddressAndPort.getByAddressOverrideDefaults(
              addressAndPort.getAddress(), addressAndPort.getPort()),
